...
async function showMsg() {
    ...
    if ($.JdFarmProdName != "") {
        if ($.JdtreeEnergy != 0) {
            ...
            if ($.JdtreeEnergy / $.JdtreeTotalEnergy >= 1) {
                console.log(`东东农场：${$.JdFarmProdName},可领取`);
            }
        } else {
        }
    }
    ...
    if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
        ...
        if (response.resultCode === '0') {
            if (response.result.medalNum == response.result.medalNum + response.result.needCollectMedalNum && response.result.medalPercent == 100) {
                console.log(`东东萌宠：${$.petInfo.goodsInfo.goodsName}，可领取。`)
            }
        }
    }
}
                        ...
                        if (data['ret'] === 0) {
                            ...
                            if (data.factoryList && data.productionList) {
                                ...
                                if (production['exchangeStatus'] === 1) {
                                    console.log(`京喜工厂：${$.jxProductName} ,已经可兑换，请手动兑换`);
                                }
                            } else {
