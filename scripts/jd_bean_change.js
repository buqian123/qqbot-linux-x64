...
!(async () => {
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            await TotalBean();
            if (!$.isLogin) {
                console.log(`京东账号${$.UserName}cookie已失效\n请重新登录获取cookie`);
                continue
            }
            await getJdZZ();
            await getMs();
...
async function showMsg() {
    ...
    ReturnMessage += `当前京豆：${$.beanCount}(今日过期${$.expirejingdou})京豆🐶\n`;
                            ...
                            if (data.factoryList && data.productionList) {
                                ...
                                if (production.investedElectric >= production.needElectric) {
                                    if (production['exchangeStatus'] === 1) {
                                        infoMsg = `${$.jxProductName} ,可兑换，请手动兑换`;
                                    }
                                    if (production['exchangeStatus'] === 3) {
                                        if (new Date().getHours() === 9) {
                                            infoMsg = `${$.jxProductName} ,已超时，请选择新商品`;
                                        }
                                    }
                                } else {
                                ...
                                if (production.status === 3) {
                                    infoMsg = `${$.jxProductName} ,已超时, 请选择重新新商品`
                                }
                            } else {
                                $.unActive = false; //标记是否开启了京喜活动或者选购了商品进行生产
                                if (!data.factoryList) {
                                    infoMsg = "当前未开始生产,请去京东APP->游戏与互动->查看更多->京喜工厂 开启活动"
    ...
    return new class {
        ...
        msg(e = t, s = "", i = "", r) {
            ...
            if (this.isMute || (this.isSurge() || this.isLoon() ? $notification.post(e, s, i, o(r)) : this.isQuanX() && $notify(e, s, i, o(r))), !this.isMuteLog) {
                let t = [""];
                t.push(e), s && t.push(s), i && t.push(i), this.logs = this.logs.concat(t)
            }
        }
        log(...t) { t.length > 0 && (this.logs = [...this.logs, ...t]) }
        logErr(t, e) {
